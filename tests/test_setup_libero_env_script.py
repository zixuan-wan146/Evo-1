...
def test_setup_libero_dry_run_uses_repo_requirements_file(tmp_path: Path):
    result = run_dry_run({"EVO1_DATA_ROOT": str(tmp_path / "data")})

    assert result.returncode == 0, result.stderr
    values = parse_key_values(result.stdout)
    assert values["EVO1_DATA_ROOT"] == str(tmp_path / "data")
    assert values["LIBERO_ENV_PREFIX"] == str(tmp_path / "data" / "envs" / "libero")
    assert values["EVO1_LIBERO_REQUIREMENTS"] == str(REPO_ROOT / "requirements-libero.txt")
...
def test_setup_libero_dry_run_accepts_custom_requirements_file(tmp_path: Path):
    ...
    result = run_dry_run(
        {
            "EVO1_LIBERO_REQUIREMENTS": str(requirements_file),
    ...
    assert result.returncode == 0, result.stderr
    values = parse_key_values(result.stdout)
    assert values["EVO1_LIBERO_REQUIREMENTS"] == str(requirements_file)
