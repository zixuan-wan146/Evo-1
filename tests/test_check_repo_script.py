...
SCRIPT = REPO_ROOT / "scripts" / "check_repo.sh"


def run_dry_run(extra_env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    env = {
        **os.environ,
        "EVO1_CHECK_DRY_RUN": "1",
        "EVO1_CHECK_SKIP_RUFF": "1",
        **(extra_env or {}),
    }
...
def test_check_repo_dry_run_lists_default_gates():
    ...
    assert "Skipping ruff because EVO1_CHECK_SKIP_RUFF=1" in output
...
def test_check_repo_dry_run_respects_skip_flags():
    result = run_dry_run(
        {
            "EVO1_CHECK_SKIP_PYTEST": "1",
            "EVO1_CHECK_SKIP_COMPILE": "1",
        }
    )

    assert result.returncode == 0, result.stderr
    output = result.stdout + result.stderr
    assert "Skipping pytest because EVO1_CHECK_SKIP_PYTEST=1" in output
    assert "Skipping compileall because EVO1_CHECK_SKIP_COMPILE=1" in output
