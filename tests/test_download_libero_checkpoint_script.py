...
def run_dry_run(extra_env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    env = {
        **os.environ,
        "EVO1_DOWNLOAD_LIBERO_CHECKPOINT_DRY_RUN": "1",
        **(extra_env or {}),
    }
...
def test_download_libero_checkpoint_dry_run_uses_data_root(tmp_path: Path):
    ...
    result = run_dry_run({"EVO1_DATA_ROOT": str(data_root)})

    assert result.returncode == 0, result.stderr
    values = parse_key_values(result.stdout)
    assert values["EVO1_LIBERO_CHECKPOINT_REPO"] == "MINT-SJTU/Evo1_LIBERO"
    assert values["EVO1_LIBERO_CHECKPOINT_DIR"] == str(data_root / "checkpoints" / "Evo1_LIBERO")
    ...
    assert values["EVO1_HF_ENDPOINT"] == ""
    assert "hf download MINT-SJTU/Evo1_LIBERO" in values["COMMAND"]


def test_download_libero_checkpoint_dry_run_applies_endpoint_only_when_requested(tmp_path: Path):
    result = run_dry_run(
        {
            "EVO1_DATA_ROOT": str(tmp_path / "data"),
            "EVO1_HF_ENDPOINT": "https://hf-mirror.com",
            "HF_MAX_WORKERS": "2",
        }
    )
    ...
    assert result.returncode == 0, result.stderr
    values = parse_key_values(result.stdout)
    assert values["EVO1_HF_ENDPOINT"] == "https://hf-mirror.com"
