...
SCRIPT = REPO_ROOT / "scripts" / "start_evo1_server.sh"
...
def run_script(ckpt_dir: Path, *, skip_preflight: bool = False) -> subprocess.CompletedProcess[str]:
    env = {
        **os.environ,
        "EVO1_PYTHON": "/bin/echo",
        "EVO1_PREFLIGHT_PYTHON": sys.executable,
        "EVO1_DEVICE": "cpu",
        "EVO1_PORT": "9010",
    }
    if skip_preflight:
        env["EVO1_SKIP_PREFLIGHT"] = "1"
...
def test_start_script_runs_checkpoint_preflight_before_server_exec(tmp_path):
    ...
    assert "scripts/Evo1_server.py" in result.stdout
...
def test_start_script_fails_when_preflight_rejects_checkpoint(tmp_path):
    ...
    assert "scripts/Evo1_server.py" not in result.stdout
