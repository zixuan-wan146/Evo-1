...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", required=True, help="Evo-1 checkpoint directory.")
    parser.add_argument(
        "--profile",
    ...
    parser.add_argument("--server-python", default="python", help="Python executable for the Evo-1 server env.")
    parser.add_argument("--libero-python", default="python", help="Python executable for the LIBERO env.")
    parser.add_argument("--host", default="127.0.0.1", help="Evo-1 server host.")
    parser.add_argument("--port", type=_port, default=9000, help="Evo-1 server port.")
    parser.add_argument("--device", default="cuda:0", help="Evo-1 server device.")
    parser.add_argument("--inference-steps", type=_positive_int, default=1, help="Evo-1 inference steps.")
...
def format_plan(plan: LiberoRunPlan) -> str:
    client_script = "scripts/run_libero_smoke.sh" if plan.kind == "smoke" else "scripts/run_libero_eval.sh"
    server_command = _command(
        [
            "env",
            f"EVO1_PYTHON={plan.server_python}",
            f"EVO1_CKPT_DIR={plan.checkpoint}",
            f"EVO1_HOST={plan.host}",
            f"EVO1_PORT={plan.port}",
            f"EVO1_DEVICE={plan.device}",
            f"EVO1_INFERENCE_STEPS={plan.inference_steps}",
    ...
    eval_command = _command(
        [
            "env",
            f"EVO1_LIBERO_PROFILE={plan.profile}",
            f"EVO1_LIBERO_RUN_DIR={plan.run_dir}",
            f"EVO1_SERVER_URI=ws://{plan.host}:{plan.port}",
