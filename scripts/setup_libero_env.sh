...
install_system_packages() {
  case "${EVO1_INSTALL_SYSTEM_PACKAGES:-auto}" in
    0|false|False|no|No)
    ...
    auto)
      if [ "$(id -u)" -ne 0 ] || ! command -v apt-get >/dev/null 2>&1; then
        log "Skipping system package installation; run as root with apt-get or set EVO1_INSTALL_SYSTEM_PACKAGES=1"
        return
      fi
      ;;
    1|true|True|yes|Yes)
      command -v apt-get >/dev/null 2>&1 || fail "apt-get is required when EVO1_INSTALL_SYSTEM_PACKAGES=1"
      ;;
    *)
      fail "Invalid EVO1_INSTALL_SYSTEM_PACKAGES=${EVO1_INSTALL_SYSTEM_PACKAGES}"
      ;;
  esac
...
download_assets() {
  ...
  if [ "${EVO1_DOWNLOAD_LIBERO_ASSETS:-1}" = "0" ]; then
...
snapshot_download(
    repo_id=os.environ.get("EVO1_LIBERO_ASSETS_REPO", "jadechoghari/libero-assets"),
    repo_type="model",
