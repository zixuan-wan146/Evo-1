...
repo_id="${EVO1_LIBERO_CHECKPOINT_REPO:-MINT-SJTU/Evo1_LIBERO}"
checkpoint_dir="${EVO1_LIBERO_CHECKPOINT_DIR:-$data_root/checkpoints/Evo1_LIBERO}"
...
if [ "${EVO1_DOWNLOAD_LIBERO_CHECKPOINT_DRY_RUN:-0}" = "1" ]; then
  printf 'EVO1_DATA_ROOT=%s\n' "$data_root"
  printf 'EVO1_LIBERO_CHECKPOINT_REPO=%s\n' "$repo_id"
  printf 'EVO1_LIBERO_CHECKPOINT_DIR=%s\n' "$checkpoint_dir"
  printf 'HF_HOME=%s\n' "$hf_home"
  printf 'HUGGINGFACE_HUB_CACHE=%s\n' "$hf_cache"
  printf 'HF_MAX_WORKERS=%s\n' "$max_workers"
  printf 'EVO1_HF_ENDPOINT=%s\n' "${EVO1_HF_ENDPOINT:-}"
  printf 'COMMAND=env HF_HOME=%q HUGGINGFACE_HUB_CACHE=%q HF_HUB_DISABLE_TELEMETRY=1 HF_HUB_DISABLE_XET=1' "$hf_home" "$hf_cache"
  if [ -n "${EVO1_HF_ENDPOINT:-}" ]; then
    printf ' HF_ENDPOINT=%q' "$EVO1_HF_ENDPOINT"
  fi
...
if [ -n "${EVO1_HF_ENDPOINT:-}" ]; then
  env_args+=("HF_ENDPOINT=$EVO1_HF_ENDPOINT")
fi
