...
def compute_normalization_stats_from_minmax(jsonl_path):
...
class SimulationDataset(Dataset):
    def __init__(
        ...
        self.cache_dir = Path(cache_dir) if cache_dir is not None else Path(os.getenv("EVO1_CACHE_DIR", DEFAULT_CACHE_DIR))
        ...
        for arm_name, arm_config in self.config['data_groups'].items():
            ...
            for dataset_name, dataset_config in arm_config.items():
                ...
                if stats_path_after_compute.exists():
                    ...
                    stats = compute_normalization_stats_from_minmax(stats_path)
