...
def validate_dataset_path(
    ...
    *,
    ...
    issues.extend(validate_stats_files(stats_json_path, episodes_stats_path, dataset_label, max_state_dim, max_action_dim))
...
def validate_stats_files(
    ...
    if stats_json_path.exists():
        ...
        return _validate_stats_payload(payload, str(stats_json_path), dataset_label, max_state_dim, max_action_dim)
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if not isinstance(stats, dict):
            ...
            continue
        issues.extend(_validate_stats_payload(stats, str(episodes_stats_path), f"{dataset_label} line {index}", max_state_dim, max_action_dim))
    return issues
...
def _validate_stats_payload(
    ...
    issues: list[DatasetValidationIssue] = []
    issues.extend(validate_minmax_stat(stats, "observation.state", max_state_dim, path_label, dataset_label))
    issues.extend(validate_minmax_stat(stats, "action", max_action_dim, path_label, dataset_label))
    return issues
...
def validate_minmax_stat(
    ...
    if not isinstance(stat, Mapping):
        return [DatasetValidationIssue("FAIL", path_label, f"{dataset_label} stats missing {stat_name} min/max object")]

    mins = stat.get("min")
    maxs = stat.get("max")
    issues: list[DatasetValidationIssue] = []
    issues.extend(validate_numeric_vector(mins, f"{stat_name}.min", max_dim, path_label, dataset_label))
    issues.extend(validate_numeric_vector(maxs, f"{stat_name}.max", max_dim, path_label, dataset_label))
    if issues:
        return issues

    if len(mins) != len(maxs):
        return [DatasetValidationIssue("FAIL", path_label, f"{dataset_label} {stat_name}.min and max must have the same length")]
    for index, (min_value, max_value) in enumerate(zip(mins, maxs)):
        if float(min_value) > float(max_value):
            issues.append(DatasetValidationIssue("FAIL", path_label, f"{dataset_label} {stat_name}.min[{index}] must be <= max[{index}]"))
    return issues
...
def validate_video_paths(
    ...
    for view_key, view_folder in view_map.items():
        if not isinstance(view_folder, str) or not view_folder:
            issues.append(DatasetValidationIssue("FAIL", str(dataset_path), f"{dataset_label} view {view_key!r} has invalid folder"))
            continue
        for parquet_path in parquet_files:
            video_path = dataset_path / "videos" / parquet_path.parent.name / view_folder / f"{parquet_path.stem}.mp4"
            if not video_path.exists():
                issues.append(DatasetValidationIssue("FAIL", str(video_path), f"{dataset_label} missing video for {parquet_path}"))
    return issues
