...
class LiberoClientConfig:
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> "LiberoClientConfig":
        ...
        ckpt_name = environ.get("EVO1_LIBERO_CKPT_NAME", "Evo1_libero_all")
        ...
        config = cls(
            horizon=env_int(environ, "EVO1_LIBERO_HORIZON", 14),
            max_steps=max_steps,
            server_url=environ.get("EVO1_SERVER_URI", environ.get("EVO1_LIBERO_SERVER_URL", "ws://127.0.0.1:9000")),
            ckpt_name=ckpt_name,
            ...
            mujoco_gl=environ.get("EVO1_MUJOCO_GL", "osmesa"),
        )
        config.validate()
        return config
    ...
    def validate(self) -> None:
        if self.horizon <= 0:
            raise ValueError(f"EVO1_LIBERO_HORIZON must be positive, got {self.horizon}")
        if self.num_episodes <= 0:
            raise ValueError(f"EVO1_LIBERO_EPISODES must be positive, got {self.num_episodes}")
        if self.task_limit < 0:
            raise ValueError(f"EVO1_LIBERO_TASK_LIMIT must be non-negative, got {self.task_limit}")
        invalid_max_steps = [value for value in self.max_steps if value <= 0]
        if invalid_max_steps:
            raise ValueError(f"EVO1_LIBERO_MAX_STEPS values must be positive, got {invalid_max_steps}")
        if self.mujoco_gl not in {"osmesa", "egl", "glfw"}:
            raise ValueError(f"EVO1_MUJOCO_GL must be one of osmesa, egl, glfw; got {self.mujoco_gl!r}")
